...
export default function SupporterCRM({ tab }) {
  ...
  function loadServerRows(searchParams, page, pageSize, tab) {
    if (tab === 2){
      return new Promise((resolve) => {
        let url = "http://localhost:3000/api/supporters/getData?";
  ...
  const validateUpdate = (params, event) => {
    ...
    if (params.field === "name" && validator.isEmpty(cellValue)) {
    ...
    } else if (params.field === "date_gifted" && validator.isEmpty(cellValue)) {
      event.defaultMuiPrevented = true;
      ...
      params.field === "gift_provided" &&
      validator.isEmpty(cellValue)
    ) {
      event.defaultMuiPrevented = true;
      enqueueSnackbar("Please enter a Gift Provided!", {
        variant: "error",
        autoHideDuration: 15000,
        action,
      }); 
  ...
  const submitChanges = () => {
    setLoading(true);
    const url = `http://localhost:3000/supporters/updateInfo`;
  ...
  return (
    ...
    >
      ...
      <Grid container display="flex" direction="row">
        <Typography sx={{ fontSize: 48, mb: "1px" }}>Supporters CRM</Typography>
      </Grid>
      <Grid display="flex">
        <Select
