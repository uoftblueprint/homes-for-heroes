...
export default function PartnerCRM({ tab }) {
  ...
  const validateUpdate = (params, event) => {
    ...
    if (params.field === "org_name" && validator.isEmpty(cellValue)) {
    ...
    } else if (params.field === "city" && validator.isEmpty(cellValue)) {
      event.defaultMuiPrevented = true;
      enqueueSnackbar("Please enter a valid city!", {
        variant: "error",
        autoHideDuration: 15000,
        action,
      });
    } else if (
      params.field === "village" &&
  ...
  const submitChanges = () => {
    setLoading(true);
    const url = `http://localhost:3000/partners/updateInfo`;
  ...
  return (
    ...
    >
      ...
      <Grid container display="flex" direction="row">
        <Typography sx={{ fontSize: 48, mb: "1px" }}>Partners CRM</Typography>
      </Grid>
      <Grid display="flex">
        <Select
