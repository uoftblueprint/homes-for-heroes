        ...
        if (err) reject(err);
          ...
          sql.query('SELECT LAST_INSERT_ID() as admin_id', (err, rows) => {
            if (err) reject(err);
              ...
              sql.query(
                'INSERT INTO admin_users (user_id, default_password, chapter_id) VALUES (?, ?, ?)',
...
Admin.listAll = function () {
  return new Promise(function (resolve, reject) {
    sql.query(
      'SELECT c.name, c.role_id, a.chapter_id FROM admin_users a INNER JOIN client_users c ON a.user_id = c.user_id',
      function (err, admins) {
        if (err) reject(err);
        else {
          resolve(admins);
...
Admin.listByChapter = function (chapter_id) {
  return new Promise(function (resolve, reject) {
    sql.query(
      'SELECT c.name FROM admin_users a INNER JOIN client_users c ON a.user_id = c.user_id where c.role_id = 1 and a.chapter_id = ?',
      [chapter_id],
      function (err, admins) {
        if (err) reject(err);
        else {
